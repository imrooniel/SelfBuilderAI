--[[
  ...
  Every module loaded through this system is:
    ...
    4. Committable to git before being activated (via git_utils if available)
...
function M.reload(name)
  ...
  if not entry then
    -- Not previously loaded — just load it fresh
    return M.load(name)
  end

  local path = entry.path

  -- Read new source for the registry snapshot
  local new_source = ""
  local f = io.open(path, "r")
  if f then new_source = f:read("*a"); f:close() end

  -- Evict from Lua's module cache
  package.loaded[name] = nil

  local ok, result = pcall(require, name)
  if not ok then
    -- Rollback: restore old module in cache
...
-- ---------------------------------------------------------------------------
-- source — return the source text of a registered module (for AI reading)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- write_and_reload — write new source to disk, commit, then hot-reload.
-- Used by self_improve.lua when the AI produces a rewritten module.
-- Returns true on success, false + err_msg on failure (source is NOT written
-- if the new code fails to load).
-- ---------------------------------------------------------------------------
